...
export default function MainApp({ onBack }: MainAppProps) {
  ...
  const handleTouchEnd = useCallback(() => {
    ...
    // Swipe left to close (> 80px)
    if (diff > 80 && isSidebarOpen) {
      setIsSidebarOpen(false);
    }
  }, [isSidebarOpen]);

  // Swipe right from edge to open sidebar (on main content area)
  ...
  const handleMainTouchEnd = useCallback((e: React.TouchEvent) => {
    ...
    if (diff > 80 && touchStartX.current < 30 && !isSidebarOpen) {
  ...
  return (
    <div className="flex h-full w-full flex-col bg-neo-gray overflow-hidden">

      {/* ── Header ── */}
      <header className="flex h-12 md:h-16 items-center justify-between border-b-2 sm:border-b-4 border-black bg-white px-3 md:px-6 z-30 shrink-0">
      ...
      <div className="flex flex-1 overflow-hidden relative">
        ...
        <aside
          ...
          className={`
            ...
            h-full w-[280px] md:w-72
            border-r-2 sm:border-r-4 border-black bg-white
        ...
        >
          <div className="flex h-full flex-col">
            <div className="p-3 md:p-4 border-b-2 sm:border-b-4 border-black bg-neo-yellow">
            ...
            <nav className="flex-1 overflow-y-auto p-3 md:p-4 space-y-4 md:space-y-6 scrollbar-thin overscroll-contain">
              {Object.entries(
                  ...
                  <div className="space-y-1">
                    {algos.map((a) => (
                      <button
                        ...
                        className={`
                          w-full text-left px-3 py-2.5 sm:py-2 font-display font-bold text-xs md:text-sm uppercase transition-all border-2
                          ${activeAlgo === a.id
                            ? 'bg-neo-blue text-white border-black shadow-neo-sm translate-x-[-2px] translate-y-[-2px]'
                            : 'bg-white text-black border-transparent hover:border-black hover:bg-gray-50 active:bg-gray-100'
                          }
                        `}
                      >
                        {a.title}
            ...
            <div className="p-3 md:p-4 border-t-2 sm:border-t-4 border-black bg-neo-gray">
        ...
        {/* ── Main Content ── */}
        <main
          className="flex-1 flex flex-col min-w-0 overflow-hidden bg-[radial-gradient(#e5e7eb_1px,transparent_1px)] [background-size:20px_20px]"
          onTouchStart={handleMainTouchStart}
          onTouchEnd={handleMainTouchEnd}
        >
          <div className="flex-1 p-2 sm:p-4 flex flex-col gap-2 sm:gap-4 overflow-y-auto overscroll-contain scrollbar-thin">
            ...
            <div className="flex flex-col sm:flex-row sm:items-center gap-2 sm:gap-4 shrink-0">
              ...
              <div className="flex-1 min-w-0">
                <h2 className="text-lg sm:text-2xl md:text-3xl font-display font-black uppercase tracking-tight truncate">
                  {algo.title}
                </h2>
            ...
            <div className="flex flex-col lg:flex-row gap-2 sm:gap-4 flex-1 min-h-0">
              ...
              {/* Visualizer */}
              <Panel title="Neural Visualization Matrix" className="flex-[3] bg-white relative overflow-hidden min-h-[220px] sm:min-h-[320px]">
                <div className="h-full w-full flex items-center justify-center">
                  {visualizers[activeAlgo]}
                </div>
              </Panel>

              {/* Info Panel — scrolls below on mobile, scrolls within on desktop */}
              <div className="flex-1 flex flex-col gap-2 sm:gap-4 lg:overflow-y-auto lg:pr-2 scrollbar-thin">
                ...
                <Panel title="Algorithm Insights" className="bg-white border-2 sm:border-4 border-black shadow-neo-xs sm:shadow-neo-sm">
                  <div className="space-y-3 sm:space-y-4">
                    <div className="p-2.5 sm:p-3 bg-neo-yellow/10 border-2 sm:border-4 border-black shadow-[2px_2px_0px_0px_rgba(0,0,0,1)] sm:shadow-[4px_4px_0px_0px_rgba(0,0,0,1)]">
                      <div className="flex items-center gap-2 mb-1">
                        <Info className="text-neo-blue shrink-0" size={14} strokeWidth={3} />
                    ...
                    <div className="grid grid-cols-2 lg:grid-cols-1 gap-2 sm:gap-3">
                      <div className="bg-neo-blue text-white p-2.5 sm:p-3 border-2 sm:border-4 border-black shadow-[2px_2px_0px_0px_rgba(0,0,0,1)] sm:shadow-[4px_4px_0px_0px_rgba(0,0,0,1)] flex items-center justify-between">
                        <div className="min-w-0">
                          <div className="text-[9px] sm:text-[10px] font-mono font-black text-white/70 uppercase">Avg_Complexity</div>
                          <div className="text-base sm:text-2xl font-black font-mono tracking-tighter truncate">{algo.timeComplexity.average}</div>
                        </div>
                        <Activity className="text-white/30 shrink-0" size={20} strokeWidth={3} />
                      </div>
                      <div className="bg-neo-green text-black p-2.5 sm:p-3 border-2 sm:border-4 border-black shadow-[2px_2px_0px_0px_rgba(0,0,0,1)] sm:shadow-[4px_4px_0px_0px_rgba(0,0,0,1)] flex items-center justify-between">
                        <div className="min-w-0">
                          <div className="text-[9px] sm:text-[10px] font-mono font-black text-black/50 uppercase">Space_Usage</div>
                          <div className="text-base sm:text-2xl font-black font-mono tracking-tighter truncate">{algo.spaceComplexity}</div>
                        </div>
                        <Cpu className="text-black/30 shrink-0" size={20} strokeWidth={3} />
                      </div>
                    </div>
                    ...
                    {algo.steps && (
                      <div className="space-y-2">
                        ...
                        <div className="space-y-1 sm:space-y-2">
                          {algo.steps.map((s, i) => (
                            <div key={i} className="flex gap-2 sm:gap-3 items-start p-2 border-2 border-black bg-white hover:bg-neo-pink/5 active:bg-neo-pink/10 transition-colors">
                ...
                <Panel title="Source Code" className="bg-neo-black text-neo-green font-mono text-xs sm:text-sm border-2 sm:border-4 border-black shadow-neo-xs sm:shadow-neo-sm">
                  ...
                  <pre className="whitespace-pre-wrap leading-relaxed text-[10px] sm:text-xs overflow-x-auto scrollbar-thin">
                    {algo.pseudoCode}
                  </pre>
                </Panel>
                ...
                <Panel title="Case Analysis" className="bg-white border-2 sm:border-4 border-black shadow-neo-xs sm:shadow-neo-sm">
                  <div className="space-y-2">
                    <div className="flex items-center justify-between text-xs p-2 bg-neo-green/10 border-2 border-black">
                      ...
                      <code className="bg-neo-green text-black px-2 py-0.5 border-2 border-black font-black text-[10px] sm:text-xs">{algo.timeComplexity.best}</code>
                    </div>
                    <div className="flex items-center justify-between text-xs p-2 bg-neo-blue/10 border-2 border-black">
                      ...
                      <code className="bg-neo-blue text-white px-2 py-0.5 border-2 border-black font-black text-[10px] sm:text-xs">{algo.timeComplexity.average}</code>
                    </div>
                    <div className="flex items-center justify-between text-xs p-2 bg-neo-red/10 border-2 border-black">
                      ...
                      <code className="bg-neo-red text-white px-2 py-0.5 border-2 border-black font-black text-[10px] sm:text-xs">{algo.timeComplexity.worst}</code>
                    </div>
                  </div>
                </Panel>
