...
export default function IntroScene({ onComplete }: { onComplete: () => void }) {
  return (
    <div className="relative h-screen w-full bg-neo-pink flex flex-col items-center justify-center overflow-hidden">
      ...
      <div className="absolute top-10 left-10 w-40 h-40 border-8 border-black bg-neo-blue -rotate-12" />
      <div className="absolute bottom-20 right-10 w-56 h-56 border-8 border-black bg-neo-green rotate-12" />
      <div className="absolute top-1/4 right-1/4 w-24 h-24 border-8 border-black bg-neo-yellow rounded-full -rotate-45" />
      <div className="absolute bottom-1/4 left-1/4 w-32 h-32 border-8 border-black bg-neo-orange rotate-45" />
      ...
      <motion.div 
        ...
        className="relative z-10 bg-white border-8 border-black p-12 shadow-[16px_16px_0px_0px_rgba(0,0,0,1)] max-w-2xl w-full mx-4"
      >
        <div className="flex flex-col items-center text-center gap-8">
          <motion.div 
            animate={{ rotate: [0, 10, -10, 0], scale: [1, 1.1, 1] }}
            transition={{ repeat: Infinity, duration: 3 }}
            className="scale-[2] mb-4"
          >
            <Logo />
          </motion.div>
          
          <div className="space-y-2">
            <h1 className="text-7xl font-display font-black uppercase tracking-tighter leading-none">
              DSA <span className="text-neo-blue">MONKEY</span>
            </h1>
            <p className="font-mono text-sm font-bold uppercase bg-neo-yellow inline-block px-2 py-1 border-2 border-black rotate-1">
          ...
          <Button 
            onClick={onComplete}
            variant="dark"
            className="w-full py-6 text-2xl group relative overflow-hidden"
          >
      ...
      <div className="absolute bottom-6 left-6 font-mono text-[10px] font-bold uppercase">
        © 2026 SD_LABS // ALL_RIGHTS_RESERVED
      </div>
      <div className="absolute bottom-6 right-6 font-mono text-[10px] font-bold uppercase">
        V1.0.5
      </div>
    </div>
  );
